...
router.get('/', async (req, res, next) => {
    try {
        ...
        const [items, total] = await Promise.all([
            prisma_js_1.default.project.findMany({ where, orderBy: { createdAt: 'desc' }, take, skip }),
            prisma_js_1.default.project.count({ where }),
        ]);
...
router.get('/:id', async (req, res, next) => {
    try {
        const item = await prisma_js_1.default.project.findUnique({ where: { id: req.params.id } });
        if (!item)
...
router.delete('/:id', async (req, res, next) => {
    try {
        await prisma_js_1.default.project.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch (e) {
        if (e?.code === 'P2025')
